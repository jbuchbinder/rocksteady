...
/**
 * This is a wrapper bean arround the Esper Complex Event Processing engine.  The manager allows for Esper
 * to be managed and initialized by the Spring container.
 *
 * NOTE:  By default Esper threading is not enabled. This is to make sure that all processing
 *        happen in the calling thread.  For example if you send an event and expect to read the
 *        results within the same thread.
 *
 * NOTE: Esper relies on ThreadLocal variables to store transient data.
 *       There is a potential of a memory leak if the engine is reset in
 *       different thread than the one used for sending events.  For example
 *       currently all events are sent by the L1 consumer.  We need to make sure that we
 *       cause the L1 Container to refresh the underlying TaskExecutor (i.e the thread factory) so as
 *       older threads are released and all associated ThreadLocal variables are GC-ed properly.
 */
public class ComplexEventManager implements Service, Startable {
    ...
    /**
     * Get the class paths where the CEP engine will look for
     * event objects
     *
    ...
    /**
     * Set the class paths array where CEP engine will look for
     * event objects
     *
    ...
    public void start() {
        if (!initialized || instance == null) {
            throw new IllegalStateException("Please initialized the Complex Event Manager before starting it.");
        }
        ...
        if ( this.dataSource != null ) {
        	...
        	configDB.setDriverManagerConnection(dataSource.getClass().getName(), dataSource.getUrl(), dataSource.getUsername(), dataSource.getPassword());
        ...
        if (enableThreading) {
            ...
            config.getEngineDefaults().getThreading().setThreadPoolInboundNumThreads(inBoundThreadPoolSize);
            config.getEngineDefaults().getThreading().setThreadPoolTimerExec(true);
            config.getEngineDefaults().getThreading().setThreadPoolTimerExecNumThreads(timmerThreadPoolSize);
        }
    ...
    private void addEplStatements() {
        ...
        for (StatementBean statementBean : statementBeans) {
            ...
            if (statementBean.getValues() != null && statementBean.getValues().size() > 0) {
                EPPreparedStatement prepared = epServiceProvider.getEPAdministrator().prepareEPL(statementBean.getEPL());
                int index = 1;
                for (Object value : statementBean.getValues()) {
                    prepared.setObject(index, value);
                    index++;
                }
                epStatement = epServiceProvider.getEPAdministrator().create(prepared, statementBean.getName());
            } else {
                epStatement = epServiceProvider.getEPAdministrator().createEPL(statementBean.getEPL(), statementBean.getName());
            }
    ...
    public void reset() {
        ...
        /* Esper relies on ThreadLocal variables to store transient data.  There is a potential
           of a memory leak if the engine is reset in different thread than the one used for sending events.
           For example currently all events are sent by the L1 consumer.  We need to make sure that we
           cause the L1 Container to refresh the underlying TaskExecutor (i.e the thread factory) so as
           older threads are released and all associated ThreadLocal variables are GC-ed properly.
         */
//        MessagingManager.getInstance().refreshListener("levelOneMessageContainer", false);
    ...
    /**
     * Set to true to enable threading in the underlying esper
     * engine.
