...
//import com.admob.rocksteady.event.Message;
...
public class MessageManager implements Service, Startable  {
	...
	// We are doing this here so these managers are started by spring manager instead of just being kicked off by spring randomly.
		...
		while ( true ) {
			try {
				...
				if ( delivery.getEnvelope().getExchange().equals(rabbitExchange) ) {
					// WE HAVE A DELIVERY!!!
					// We going to pass the message to another object and let it handle the parsing.
	...
	private void connectRabbit() {
		...
		try {
			...
			// Add a random string to queue name so each instance of rocksteady will have its own queue
			Random r = new Random();
			...
			// Create queue, passive = true, Durable setup externally, exclusive = false, AutoDelete externally.
